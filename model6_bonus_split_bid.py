...
class Model6Bonus_split_bid:
    """
    Single-hour co-optimized reserve and day-ahead market clearing.
    """
    ...
    def __init__(
        ...
        """
        Runs and stores co-optimized reserve and day-ahead market clearing at a given hour.
        ...
        Attributes:
            ...
            combined_model:          Solved Gurobi model for co-optimized clearing.
            out_dict (dict):         Parsed output from both markets. Only set if
        ...
        self.combined_model = self._run_model_combined_clearing(
            up_balance_multiplier, down_balance_multiplier
        )
        # Keep aliases for compatibility with existing downstream code.
        self.reserve_model = self.combined_model
        self.day_ahead_model = self.combined_model

        if self.combined_model.status == gp.GRB.OPTIMAL:
            self.out_dict = self._analyze_output()
            self._plot_merit_order()

    def _run_model_combined_clearing(
        self,
        up_balance_multiplier: float = 0.15,
        down_balance_multiplier: float = 0.1,
        reserve_subset: Optional[set[int]] = None,
    ) -> gp.Model:
        """
        Build and solve a single co-optimized day-ahead and reserve clearing model.

        Maximizes social welfare:
            demand utility
            - day-ahead generation cost
            - reserve procurement cost

        subject to:
            - Generator dispatch-reserve coupling: p_gen <= P_max - r_up, p_gen >= r_down
            - Generator reserve limits: r_up <= R_plus, r_down <= R_minus
            - Combined generator cap: r_up + r_down <= P_max
            - Wind, demand, power balance and reserve balance constraints

        Returns:
            gp.Model: Solved Gurobi model. Check `.status == GRB.OPTIMAL`
                      before extracting results.
        """
        model = gp.Model("model6_combined")
        ...
        if reserve_subset is None:
            reserve_subset = set(self.P_gens.keys())
        ...
        for i in self.P_gens:
            ...
            p_gen_base[i] = model.addVar(
                vtype=GRB.CONTINUOUS,
                lb=0,
                ub=self.P_gens[i]["P_max"],
                name=f"p_gen_{i}",
            )
        ...
        model.setObjective(
            ...
            # Split-bid logic: the base block tied to down-reserve is costless in DAM;
            # only the flexible block is charged at marginal generation cost.
            - gp.quicksum(self.P_gens[i]["C"] * p_gen_flex[i] for i in self.P_gens)
            - (
        ...
        # Define Constraints
        # physical and reserve coupling constraints for conventional generators
        for i in self.P_gens:
            model.addConstr(
                p_gen_base[i] + p_gen_flex[i] <= self.P_gens[i]["P_max"] - r_gen_up[i],
                name=f"max_power_gen_{i}",
            )
            model.addConstr(p_gen_base[i] == r_gen_down[i], name=f"base_equals_down_res_{i}")
        ...
        model.addConstr(
            ...
            - gp.quicksum(p_gen_base[i] + p_gen_flex[i] for i in self.P_gens)
    ...
    def _analyze_output(self) -> dict:
        ...
        """
        # Reserve Market (RM)
        # In a maximization problem, dual values for these equalities are the opposite
        # sign compared with the prior reserve-cost minimization formulation.
        ...
        def _get_reserve_val(var_name):
            var = self.combined_model.getVarByName(var_name)
        ...
        reserve_procurement_cost = sum(
            self.P_gens[i]["C_up"] * _get_reserve_val(f"r_gen_up_{i}")
            + self.P_gens[i]["C_down"] * _get_reserve_val(f"r_gen_down_{i}")
            for i in self.P_gens
        )
        ...
        for i in self.P_gens:
            ...
            flex = self.combined_model.getVarByName(f"p_gen_flex_{i}").X
            base_schedule[f"gen_{i}"] = base
            flex_schedule[f"gen_{i}"] = flex
        ...
        cost_producers_DAM = {
            f"gen_{i}": flex_schedule[f"gen_{i}"] * self.P_gens[i]["C"]
            for i in self.P_gens
        }

        utility_demands = [
            self.combined_model.getVarByName(f"demand_{i}").X
            * (self.Demands["bidding_prices"][i] - dam_clearing)
            for i in self.Demands["load_distribution"]
        ]

        total_operating_cost = sum(
            self.combined_model.getVarByName(f"p_gen_flex_{i}").X * self.P_gens[i]["C"]
            for i in self.P_gens
        )

        social_welfare_excl_reserve = social_welfare + reserve_procurement_cost
        ...
        out_dict["day_ahead_market"] = {
            ...
            "social_welfare_excluding_reserve_cost": social_welfare_excl_reserve,
            "total_operating_cost": total_operating_cost,
            "reserve_procurement_cost": reserve_procurement_cost,
            "scheduled_power_output": dam_schedule,
            "scheduled_power_output_base": base_schedule,
            "scheduled_power_output_flex": flex_schedule,
    ...
    def _plot_merit_order(self):
        ...
        for cap, cost, label, is_forced in zip(
            ...
            if is_wind:
                ...
                forced_min = reserve_down.get(label, 0.0) if isinstance(label, int) else 0.0
        ...
        # ax.set_title(
        #     "Day-Ahead Market — Merit Order Curve after Reserve Market (Step 6)",
        #     fontsize=23,
        #     fontweight="bold",
        # )
        ...
        # reserve_up = {
        #     i: self.reserve_model.getVarByName(f"r_gen_up_{i}").X for i in self.P_gens
        # }
        ...
        # ax.set_title(
        #     "Day-Ahead Market — Merit Order Curve", fontsize=14, fontweight="bold"
        # )
        # ax.set_xlim(0, max(total_capacity, x_cursor_d) * 1.02)
        # ax.set_ylim(-0.5, max(all_costs) * 2 + 1)
        # ax.grid(axis="y", linestyle=":", linewidth=0.7, alpha=0.7, zorder=1)
